...
//        Map<String, MultipartFile> fileMap = multipartRequest.getFileMap();
        ...
        boolean excel = isExcel(file.getOriginalFilename());
        if (excel==false) return Result.error("文件格式不对");
//        String fullyFileName = file.getName();//获取文件名称和后缀
//
//        String[] split = fullyFileName.split(".");
...
//        }
//        String type = split[split.length - 1];//文件类型
//
//        //文件临时路径
...
//                type;
    ...
    private boolean isExcel(String orgName) {
        ...
        String fn = orgName.substring(0,orgName.indexOf("."));
...
//        String type = split[split.length - 1];//文件类型
    ...
    private String uploadLocal(MultipartFile mf, String bizPath) {
        try {
            ...
            System.out.println("111解析的list对象为" + list);
            log.info("222解析的list对象为" + list);
            map.put("list",list);
            map.put("sysNo",sysNo);
